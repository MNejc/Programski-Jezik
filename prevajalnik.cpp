...
void spremenljivka(string &s, int &ind) {
    ...
    izhod[tI] << "\tmov rax, [" << glob[spre[zeton].glob] << " - " << spre[zeton].pol*8 << "]" << endl;
}
...
void nastaviSpremenljivko(string &s, int &ind, string ime) {
    ...
    izhod[tI] << "\tmov [" << glob[spre[ime].glob] << " - " << spre[ime].pol * 8 << "], rax" << endl;
}
...
void zaZanka(string &s, int &ind) {
    ...
    if (zeton == "od") {
        ...
        izhod[tI] << "\tmov [" << glob[spre[ime].glob] << " - " << spre[ime].pol * 8 << "], rax\n" << endl;
    ...
    izhod[tI] << "\tmov rax, [" << glob[spre[ime].glob] << " - " << spre[ime].pol*8 << "]\n\tdec rax\n\tmov [" << glob[spre[ime].glob] << " - " << spre[ime].pol*8 << "], rax\nzanka" << stVej << ":\n\tmov rax, [" << glob[spre[ime].glob] << " - " << spre[ime].pol*8 << "]\n\tinc rax\n\tmov [" << glob[spre[ime].glob] << " - " << spre[ime].pol*8 << "], rax" << endl;

    izraz(s, ind);
    izhod[tI] << "\tmov rbx, [" << glob[spre[ime].glob] << " - " << spre[ime].pol * 8 << "]\n\tcmp rax, rbx\n\tjl konec" << stVej << endl;
}
...
void izhodIzFunkcije(string &s, int &ind) {
    ...
    izraz(s, ind);
    izhod[tI] << "\tmov [" << glob[spre[trenFunk].glob] << " - " << spre[trenFunk].pol * 8 << "], rax" << endl;
